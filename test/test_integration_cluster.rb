...
class TestIntegrationCluster < TestPuma::ServerSpawn
  ...
  def test_fork_worker_phased_restart_with_high_worker_count
    worker_count = 10
    # intermittent failures waiting for 'Ctrl-C'
    timeout = RUBY_VERSION < '2.5' && ::Puma::IS_OSX ? 15 : nil

    server_spawn "test/rackup/hello.ru", timeout: timeout, config: <<~CONFIG
      fork_worker 0
      worker_check_interval 1
  ...
  def test_culling_strategy_oldest
    ...
    get_worker_pids # to consume server log
