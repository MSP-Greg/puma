...
class WebServerTest < Minitest::Test
  ...
  def test_requests_count
    ...
    3.times { send_http_read_response GET_10 }
    assert_equal @server.requests_count, 3
  end

  def test_trickle_attack
    response = do_test(VALID_REQUEST, 3)
  ...
  def test_bad_client
    response = do_test("GET /test HTTP/BAD", 3)
    assert_includes response, "Bad Request"
  end

  def test_bad_path
    response = do_test("GET : HTTP/1.1\r\n\r\n", 3)
  ...
  def test_file_streamed_request
    ...
    response = do_test(long, (Puma::Const::CHUNK_SIZE * 2) - 400)
    assert_includes response, "hello"
  end

  def test_supported_http_method
    response = do_test("PATCH www.zedshaw.com:443 HTTP/1.1\r\nConnection: close\r\n\r\n", 100)
    assert_includes response, "hello"
  end

  def test_nonexistent_http_method
    response = do_test("FOOBARBAZ www.zedshaw.com:443 HTTP/1.1\r\nConnection: close\r\n\r\n", 100)
    assert_includes response, "Not Implemented"
  end
