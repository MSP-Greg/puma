...
# Don't run parallel, some tests change ENV
...
class TestCLI < TestPuma::ServerInProcess
  ...
  def test_plugins
    plugins = Puma::Plugins.instance_variable_get(:@plugins)
    assert_empty plugins

    cli_new ['--plugin', 'tmp_restart', '--plugin', 'systemd']

    assert_equal ['tmp_restart', 'systemd'], plugins.keys
  ensure
    plugins.clear
  end
end
