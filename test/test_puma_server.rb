...
class TestPumaServer < TestPuma::ServerInProcess
  ...
  def test_very_large_return
    ...
    out = send_http_read_body GET_10
  ...
  def test_immediate_pipeline_not_confused_for_body
    ...
    response = socket.timed_sysread timeout: 3
  ...
  def test_lowlevel_error_body_close
    ...
    body = response.body
    assert_match(/Puma caught this error: undefined method [`']to_i' for/, body)
    assert_includes body, "Array"
    refute_includes body, 'lowlevel_error'
  ...
  def test_lowlevel_error_message
    ...
    assert_includes response, "HTTP/1.0 500 #{STATUS_CODES[500]}"
  ...
  def test_lowlevel_error_message_without_backtrace
    ...
    assert_includes response.body, 'Puma caught this error: no backtrace error (WithoutBacktraceError)'
    assert_includes response.body, '<no backtrace available>'
  end
  ...
  def test_force_shutdown_error_default
    ...
    assert_start_with response, 'HTTP/1.0 503 Service Unavailable'
    assert_match(/Puma caught this error.+Puma::ThreadPool::ForceShutdown/, response.body)
  end
  ...
  def test_prints_custom_error
    ...
    assert_start_with response, 'HTTP/1.0 302 Found'
  end
  ...
  def test_run_stop_thread_safety
    ...
    100.times do |i|
      ...
      Thread.pass
      @server.stop
      assert thread.join(1), "Failed on iteration #{i+1} of 100"
    end
  end
  ...
  def test_remote_address_header
    ...
    assert_equal HOST, send_http_read_body
  end
