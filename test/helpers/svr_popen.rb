...
module TestPuma
  class SvrPOpen < SvrBase
    ...
    CTRL_TYPES = %i[pid pid_file state_pid state_tcp state_aunix state_unix tcp] + BIND_TYPES
    ...
    def workers(qty = 'no')
      if qty == 'no'
        ...
        raise ArgumentError, 'Parameter (#{qty.inspect}) be an Integer or nil'
      end
    end
    ...
    def cmd_to_log_str(cmd)
      ...
      worker_boot = " - Worker #{workers - 1} (PID: " if workers
