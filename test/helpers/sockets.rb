...
module TestPuma
  ...
  module SktPrepend
    ...
    def get_body(path = nil, dly: nil, body_conf: nil, timeout: nil)
      fast_write get_req(path, dly: dly, body_conf: body_conf)
      read_body timeout
    end
    ...
    def get_response(path = nil, dly: nil, body_conf: nil, timeout: nil)
      fast_write get_req(path, dly: dly, body_conf: body_conf)
    ...
    def read_response(timeout = nil)
      ...
      if to_io.wait_readable timeout
        loop do
          begin
    ...
    # Sends a get request
    # @!macro req
    def get(path = nil, dly: nil, body_conf: nil)
      fast_write get_req(path, dly: dly, body_conf: body_conf)
    end
    ...
    # Returns an HTTP/1.1 GET request string, with 'Dly:' and 'body_conf:' set for the 'ci-*'
    # rackup files.
    # @!macro req
    # @return [String]
    def get_req(path = nil, dly: nil, body_conf: nil)
      req = "GET /#{path} HTTP/1.1\r\n".dup
      req << "Dly: #{dly}\r\n" if dly
      req << "Body-Conf: #{body_conf}\r\n" if body_conf
      req << "\r\n"
    end
  end
  ...
  #
  # The `#request_stream` method creates a stream of requests, and collections data
  ...
  #    1.0  => 21.54}
  #
  # Note that `#request_stream` is not a replacement for `wrk` or other tools, as
  ...
  # `#request_stream` shows the time Puma takes to respond to a request.
  #
  ...
  module Sockets
    ...
    # macOS sometimes raises Errno::EBADF or Errno::ECONNABORTED for socket init
    # error
    #
    OPEN_WRITE_ERRORS = begin
      ary = [Errno::ECONNREFUSED, Errno::ECONNRESET, Errno::ENOENT,
        Errno::EPIPE, Errno::EBADF, IOError]
      ary << Errno::ENOTCONN << Errno::ECONNABORTED if IS_OSX
    ...
    # @!macro ret_skt
    #
    def create_socket(type: nil, p: nil)
      ...
      skt =
        ...
        when :ssl4, :ssl, :ssl6
          ...
          temp = SktSSL.new(SktTCP.new(host, port), ctx).tap { |s|
            s.sync_close = true
            s.connect
          }
    ...
    # @!macro ret_skt
    #
    def connect_get(path = nil, dly: nil, body_conf: nil)
      req = "GET /#{path} HTTP/1.1\r\n".dup
      req << "Dly: #{dly}\r\n" if dly
      req << "Body-Conf: #{body_conf}\r\n" if body_conf
      req << "\r\n"
    ...
    # @return [String] response body
    #
    def connect_get_body(path = nil, dly: nil, body_conf: nil, timeout: nil)
      connect_get(path, dly: dly, body_conf: body_conf).read_body timeout
    end
    ...
    #
    def connect_get_response(path = nil, dly: nil, body_conf: nil, timeout: nil)
      connect_get(path, dly: dly, body_conf: body_conf).read_response timeout
    end
    ...
    # @param req_str [String] request string
    # @!macro connect
    # @!macro ret_skt
    #
    def connect_raw(req_str = nil, type: nil, p: nil)
      s = create_socket type: type, p: p
      s.fast_write req_str
      s
    end
    ...
    # @param [Integer, String] body_conf: size of response body in kB
    ...
    def request_stream(replies, threads, clients_per_thread,
      req_per_client: 1, dly_thread: nil, dly_client: nil, dly_app: nil,
      body_conf: 10, keep_alive: true, resp_timeout: RESP_READ_TIMEOUT)
        ...
        connection_threads << Thread.new do
          ...
          clients_per_thread.times do |client_idx|
            ...
            req_per_client.times do |req_idx|
              ...
              begin
                if req_idx.zero?
                  socket = connect_get dly: dly_app, body_conf: body_conf
                  ...
                  socket.get dly: dly_app, body_conf: body_conf
                end
                ...
                begin
                  ...
                  if body =~ /^Hello World$/
                    ...
                    mutex_r_ok.synchronize {
                      ...
                      # no pid for File response bodies
                      body_pid = body[/\A\d+/].to_i
                      unless body_pid.zero?
                ...
                if (req_idx + 1 == req_per_client || !keep_alive) &&
                    socket && socket.to_io.is_a?(IO) && !socket.closed?
                  # STDOUT.syswrite "#{id} close\n"
                  begin
                    if @bind_type == :ssl
                      socket.sysclose
                    else
    ...
    # @param [Hash] replies the hash supplied to `request_stream`
    ...
    def replies_info(replies)
      ...
      if replies[:reqs_good_wr]
        ...
        msg << "\nwrite  %5d" % replies[:reqs_good_wr].reduce(:+)
        replies[:reqs_good_wr].each { |i| msg << "  %4d" % i }
        msg << "\n read  %5d" % replies[:reqs_good_rd].reduce(:+)
    ...
    # values are the time.
    # @param [Hash] replies the hash supplied to `request_stream`
    # @param [Integer] threads same as `request_stream` parameter
    # @param [Integer] clients_per_thread same as `request_stream` parameter
    # @param [Integer] req_per_client same as `request_stream` keyword
