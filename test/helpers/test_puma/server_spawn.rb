...
module TestPuma
  ...
  class ServerSpawn < ServerBase
    ...
    def server_spawn(argv = nil, # rubocop:disable Metrics/ParameterLists
        ...
        timeout: nil,      # timeout for server 'Ctrl-C'
    ...
    def server_gets(match_obj, timeout_time, timeout_interval, log: false)
      error_retries = 0
      line = ''

      sleep 0.05 unless @server.is_a?(IO) or Process.clock_gettime(Process::CLOCK_MONOTONIC) > timeout_time

      unless @server.is_a? IO
        raise RuntimeError,  "@server is not an IO"
      end
      if Process.clock_gettime(Process::CLOCK_MONOTONIC) > timeout_time
        raise Timeout::Error, "Waited #{timeout_interval} seconds for server to log #{match_obj.inspect}"
      end
      ...
      restart_thread = Thread.new do
        ...
        while run do
          begin
            ...
            restart_count += 1
            sleep(Puma::IS_WINDOWS ? 1.0 : 0.25)
          rescue Errno::EBADF, Timeout::Error, Errno::EINVAL
            break
          end
        end
      end
