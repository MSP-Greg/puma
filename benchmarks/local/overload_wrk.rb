...
module TestPuma
  ...
  # The following only apply when using  ci_array.ru, ci_chunked.ru, ci_io.ru,
  #   or ci_string.ru:
  ...
  # ```
  # benchmarks/local/overload_wrk.sh -b10 -c1 -T10 -s tcp -w2 -t5:5 -R test/rackup/ci_io.ru
  # ```
  # runs wrk with 5 connections per thread and use hello.ru, show object info after.
  # ```
  # benchmarks/local/overload_wrk.sh -c5 -s tcp -w4 -t5:5 -C test/config/worker_object_info.rb -R test/rackup/hello.ru
  # ```
  #
  class OverloadWrk < BenchBase

    # delay between wrk finishing and running stats
    DELAY_STATS = 5

    # thread multipliers used for wrk jobs, geometric sequence from 0.5 to 3.0
    MULTS = [
      0.5,
      0.782542290,
      1.224744871,
    ...
    def run
      ...
      threads = (@workers || 1) * (max_threads || 5).to_i
      ...
      temp.each do |t|
        @info << {
          threads: t,
          connections: t * @clients_per_thread,
          smem: nil,
          stats: nil,
          wrk: nil
        }
      ...
      @info[1..-1].each do |info|
        ...
        wrk_cmd = %Q[wrk -t#{threads} -c#{connections} -d#{@wrk_time}s --latency#{header} #{@wrk_bind_str}]
    ...
    def run_summary(info)
      str = format "%6d req/sec", info[:wrk][:rps]
    ...
    def overall_summary
      ...
      puts format("#{' ' * 12}%6d#{' ' * 37}Totals %8d %8d", @summary[:rps], reqs_puma, reqs_wrk)
      puts '═'*86
      puts 'errors - c: connection, w: write, r: read, t: timeout' if @errors
      puts ''
    end
      ...
      workers.each do |w|
        ...
        mem.each do |type|
          ...
          info_loop.each_with_index do |hsh, i|
            ...
            leaks << (requests.zero? ? 0 : 1000 * mem_increase/requests)
          end
